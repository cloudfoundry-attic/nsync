...
type Fetcher interface {
	Fetch(chan<- models.DesiredLRP) error
}
...
func (fetcher *CCFetcher) Fetch(resultChan chan<- models.DesiredLRP) error {
	return fetcher.fetchBatch(initialBulkToken, resultChan)
}

func (fetcher *CCFetcher) fetchBatch(token string, resultChan chan<- models.DesiredLRP) error {
	...
	// TODO - make sure the client is configured right
	...
	return fetcher.fetchBatch(string(*response.BulkToken), resultChan)
}
